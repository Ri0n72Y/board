...
- **运行时配置**：`apps/board-api/config/board.yaml` 是运行时配置（PidAllocator 会回写 pid 状态），被 git 忽略；模板为同目录 `board.example.yaml`（复制为 board.yaml 后自定义）。不要在 PR 里提交 board.yaml 的 pid 状态变化。
