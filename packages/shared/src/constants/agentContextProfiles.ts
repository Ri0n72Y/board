...
export const AGENT_CONTEXT_PROFILE_DEFINITIONS: readonly AgentContextProfileDefinition[] = [
  {
    ...
    description: 'Complete board context with records, relations, assets, and diagnostics.',
    source: 'both',
    level: 'full',
  ...
  },
  {
    ...
    description: 'Exports the current visible scope using the active board filters.',
    source: 'both',
    level: 'filtered',
    requiresRecord: false,
    requiresSprint: false,
  ...
  },
  {
    ...
    description: 'Includes a source card and the directly connected related records.',
    source: 'both',
    level: 'related',
    requiresRecord: true,
    requiresSprint: false,
...
const PROFILE_INDEX = new Map<AgentContextProfile, AgentContextProfileDefinition>(
  AGENT_CONTEXT_PROFILE_DEFINITIONS.map((definition) => [definition.id, definition])
)
...
export function listAgentContextProfiles(
  ...
  if (source === 'snapshot') {
    const priorityIds: AgentContextProfile[] = ['agent-snapshot', 'human-summary']
    const prioritized = priorityIds
      .map((profile) => PROFILE_INDEX.get(profile))
      .filter(
        (definition): definition is AgentContextProfileDefinition =>
          Boolean(definition)
      )
