import type { BoardCurrentQuery, BoardCurrentProjection } from '../interfaces/index.js'
...
eq(ids().includes('record-archived'), false, 'includeArchived false excludes status:archived')
eq(
  ids({ includeArchived: true }).includes('record-archived'),
  true,
  'includeArchived true includes status:archived'
)
eq(ids({ tags: ['status:wip', 'priority:urgent-important'] }).join(','), 'record-visible', 'tags all')
eq(ids({ tags: ['status:wip', 'status:todo'], tagMatch: 'any' }).join(','), 'record-visible,record-other', 'tags any')
eq(normalizeBoardFilterQuery({}).tagMatch, 'all', 'tagMatch undefined defaults to all')
eq(ids({ tags: ['status:wip', 'status:todo'] }).join(','), '', 'tagMatch undefined uses all')
eq(ids({ assignee: 'member-visible' }).join(','), 'record-visible', 'assignee')
eq(ids({ assetId: 'asset-visible' }).join(','), 'record-visible', 'assetId')
eq(ids({ relationTarget: 'record-target' }).join(','), 'record-visible', 'relationTarget')
eq(ids({ q: 'visible title' }).join(','), 'record-visible', 'q matches title')
eq(ids({ q: 'visible description' }).join(','), 'record-visible', 'q matches description')
eq(ids({ q: 'visible content' }).join(','), 'record-visible', 'q matches content')
eq(ids({ q: 'CARD-1' }).join(','), 'record-visible', 'q matches pid')
eq(ids({ q: 'record-visible' }).join(','), 'record-visible', 'q matches id')
eq(ids({ q: 'priority:urgent-important' }).join(','), 'record-visible', 'q matches tag')
eq(ids({ q: 'asset-visible' }).join(','), 'record-visible', 'q matches asset id')
eq(ids({ q: 'record-target' }).join(','), 'record-visible', 'q matches relation target')
eq(ids({ q: 'blocks' }).join(','), 'record-visible', 'q matches relation constraint')
eq(ids({ q: '  VISIBLE TITLE  ' }).join(','), 'record-visible', 'q trim / case-insensitive')
