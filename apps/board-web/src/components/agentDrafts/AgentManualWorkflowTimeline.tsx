  ...
  return (
    <div className="grid grid-cols-[16px_1fr] gap-x-3 gap-y-1">
      <div className="flex justify-center pt-1">
        <span
          className={`inline-block h-3 w-3 rounded-full ring-2 ${cls.dot}`}
        />
      </div>
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <span
            className={`rounded px-1.5 py-0.5 text-xs font-bold uppercase ${cls.badge}`}
          >
        ...
        {children && (
          <div
            className={`mt-1.5 grid gap-1 rounded-md border border-slate-200 bg-white px-3 py-2 text-xs ${cls.text}`}
          >
            {children}
          </div>
        )}
  ...
  return (
    <section className="grid gap-3 rounded-lg border border-slate-200 bg-white p-5">
      ...
      >
        ...
        <p>
          {draft.source} · {draft.profile} · {draft.recordCount}{' '}
          {t('agent.queue.records')}
          {draft.snapshotId
            ? ` · ${t('agent.meta.snapshot').toLowerCase()} ${draft.snapshotId.slice(0, 8)}`
            : ''}
        </p>
      </TimelineItem>
      ...
      {draft.status === 'draft' ? (
        ...
        <TimelineItem
          ...
          meta={
            draft.reviewedAt
              ? `${formatDate(draft.reviewedAt)} ${t('agent.timeline.tone.by', { name: draft.reviewedBy ?? t('agent.reviewInfo.unknown') })}`
              : undefined
          }
        >
          ...
          {draft.reviewNote && (
            <p className="italic">
              {t('agent.timeline.reviewedNote', { note: draft.reviewNote })}
            </p>
          )}
        </TimelineItem>
      ) : (
        <TimelineItem
          ...
          meta={
            draft.reviewedAt
              ? `${formatDate(draft.reviewedAt)} ${t('agent.timeline.tone.by', { name: draft.reviewedBy })}`
              : undefined
          }
        >
      ...
      {draft.status === 'reviewed' ? (
        ...
        <TimelineItem
          tone="blocked"
          title={t('agent.timeline.handoffDisabled')}
        >
      ...
      {responseCount > 0 ? (
        ...
        >
          ...
          <ol className="grid gap-1.5 mt-1">
            {recentResponses.map((r) => (
              ...
              >
                ...
                {r.responseNote && (
                  <span className="col-start-2 text-slate-500">
                    «{r.responseNote}»
                  </span>
                )}
              </li>
          ...
          {responseCount > 3 && (
            <p className="mt-1 text-xs text-slate-500">
              {t('agent.timeline.responsesMore', { count: responseCount - 3 })}
            </p>
          )}
        </TimelineItem>
      ) : draft.status === 'reviewed' ? (
        ...
        <TimelineItem
          tone="pending"
          title={t('agent.timeline.noResponsesLocked')}
        >
        ...
        <TimelineItem
          tone="blocked"
          title={t('agent.timeline.noResponsesBlocked')}
        >
