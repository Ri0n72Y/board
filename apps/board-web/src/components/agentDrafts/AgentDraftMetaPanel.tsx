...
export function AgentDraftMetaPanel({ draft }: AgentDraftMetaPanelProps) {
  ...
  const copyMarkdown = () => {
    navigator.clipboard.writeText(draft.contextMarkdown).then(
      () => {
        setCopyFeedback(t('agent.meta.copied'))
  ...
  const downloadMarkdown = () => {
    downloadTextFile(
      `agent-draft-${draft.id.slice(0, 8)}-${draft.status}.md`,
  ...
  return (
    <section className="grid gap-3 rounded-lg border border-slate-200 bg-white p-5">
      ...
      <dl className="grid gap-2 sm:grid-cols-2">
        ...
        <MetaItem
          label={t('agent.meta.created')}
          value={formatDate(draft.createdAt)}
        />
        <MetaItem
          label={t('agent.meta.createdBy')}
          value={draft.createdBy}
          mono
        />
        <MetaItem label={t('agent.meta.status')} value={draft.status} />
        <MetaItem
          label={t('agent.meta.records')}
          value={draft.recordCount.toString()}
        />
        <MetaItem
          label={t('agent.meta.contextGoal')}
          value={draft.contextGoal ?? t('agent.meta.none')}
        />
        {draft.snapshotId && (
          <MetaItem
            label={t('agent.meta.snapshot')}
