...
describe('RecordService patch submission (createRecordPatch)', () => {
  describe('basic flow', () => {
    it('creates a patch and returns patch + newCurrentVersion', async () => {
      ...
      expect(result!.patch.body).not.toHaveProperty('currentVersion')
  ...
  describe('snapshot head / parent chain', () => {
    ...
    it('currentVersion is not persisted to patch body', async () => {
      const service = createRecordService()
      const envelope = await service.create({
        schema: 'CardBody',
        tags: ['status:todo'],
      ...
      expect(found!.body).not.toHaveProperty('currentVersion')
    })
    ...
    it('atomically rejects concurrent patches with the same parent and currentVersion', async () => {
      ...
      const fulfilled = submissions.filter(
        (
          ...
          NonNullable<Awaited<ReturnType<RecordService['createRecordPatch']>>>
  ...
  describe('audit envelope', () => {
    ...
    it('patch createdAt is server-generated and cannot be overridden by client', async () => {
      ...
      expect(patchCreatedAt.getTime()).toBeLessThanOrEqual(
        after.getTime() + 1000
      )
...
function createConfiguredTagService(): RecordService {
  ...
  config.tags.status.custom = [{ id: 'status:doing', displayName: 'doing' }]
