...
export function createAgentRoute(
  ...
  route.get('/drafts/:id/handoff', async (c) => {
    try {
      ...
      return c.json<ApiResponse<GetAgentDraftHandoffResponse>>(ok({ handoff }), 200)
    } catch (caught) {
      if (caught instanceof AgentDraftNotFoundError) {
        return c.json(error('NOT_FOUND', caught.message), 404)
      }
  ...
  route.get('/drafts/:id', async (c) => {
    ...
    if (!draft) {
      return c.json(error('NOT_FOUND', `Agent draft ${c.req.param('id')} not found`), 404)
    }
  ...
  route.patch('/drafts/:id/review', async (c) => {
    ...
    try {
      ...
      const draft = await agentDraftService.updateReview(c.req.param('id'), input, actor)
  ...
  route.post('/drafts/:id/responses', async (c) => {
    ...
    try {
      ...
      const response = await agentResponseService.createResponse(draftId, input, actor)
      return c.json<ApiResponse<CreateAgentResponseResponse>>(ok({ response }), 201)
    } catch (caught) {
  ...
  route.get('/responses/:responseId', async (c) => {
    const response = await agentResponseService.getResponse(c.req.param('responseId'))
    if (!response) {
      return c.json(error('NOT_FOUND', `Agent response ${c.req.param('responseId')} not found`), 404)
    }
...
async function parseUpdateReviewInput(request: Request): Promise<UpdateAgentDraftReviewInput> {
  ...
  if (typeof body.status !== 'string' || !VALID_STATUSES.includes(body.status as AgentDraftStatus)) {
    throw new AgentDraftValidationError(`status is required and must be one of: ${VALID_STATUSES.join(', ')}`)
  }
...
async function parseCreateDraftInput(request: Request): Promise<CreateAgentDraftInput> {
  ...
  if (typeof body.profile !== 'string') throw new AgentDraftValidationError('profile is required')
  let profile: AgentContextProfile
  try {
    profile = getAgentContextProfileDefinition(body.profile as AgentContextProfile).id
  } catch { throw new AgentDraftValidationError(`Invalid profile: ${body.profile}`) }
  const rawSource = body.source as string | undefined
  if (!rawSource || !VALID_SOURCES.includes(rawSource as AgentDraftSource)) {
    throw new AgentDraftValidationError('source is required and must be current-board or snapshot')
  }
  ...
  const contextGoal = typeof body.contextGoal === 'string' && body.contextGoal.trim() ? body.contextGoal.trim() : undefined
  const recordId = typeof body.recordId === 'string' && body.recordId.trim() ? body.recordId.trim() : undefined
  const sprintTag = typeof body.sprintTag === 'string' && body.sprintTag.trim() ? body.sprintTag.trim() : undefined
...
function parseFilters(body: Record<string, unknown>): CreateAgentDraftInput['filters'] | undefined {
  const rawFilters = body.filters
  if (rawFilters === undefined || rawFilters === null) return undefined
  if (typeof rawFilters !== 'object' || Array.isArray(rawFilters)) throw new AgentDraftValidationError('filters must be an object')
  const f = rawFilters as Record<string, unknown>
  const filters: NonNullable<CreateAgentDraftInput['filters']> = {}
  if (f.q !== undefined) { if (typeof f.q !== 'string') throw new AgentDraftValidationError('filters.q must be a string'); if (f.q.trim()) filters.q = f.q.trim() }
  if (f.tags !== undefined) { if (!Array.isArray(f.tags)) throw new AgentDraftValidationError('filters.tags must be an array'); const tags = f.tags.filter((t): t is string => typeof t === 'string' && t.trim().length > 0); if (tags.length > 0) filters.tags = tags as Tag[] }
  if (f.tagMatch !== undefined) { if (f.tagMatch !== 'all' && f.tagMatch !== 'any') throw new AgentDraftValidationError('filters.tagMatch must be all or any'); filters.tagMatch = f.tagMatch }
  if (f.includeArchived !== undefined) { if (typeof f.includeArchived !== 'boolean') throw new AgentDraftValidationError('filters.includeArchived must be a boolean'); filters.includeArchived = f.includeArchived }
  if (f.assignee !== undefined) { if (typeof f.assignee !== 'string') throw new AgentDraftValidationError('filters.assignee must be a string'); if (f.assignee.trim()) filters.assignee = f.assignee.trim() }
  if (f.assetId !== undefined) { if (typeof f.assetId !== 'string') throw new AgentDraftValidationError('filters.assetId must be a string'); if (f.assetId.trim()) filters.assetId = f.assetId.trim() }
  if (f.relationTarget !== undefined) { if (typeof f.relationTarget !== 'string') throw new AgentDraftValidationError('filters.relationTarget must be a string'); if (f.relationTarget.trim()) filters.relationTarget = f.relationTarget.trim() }
...
async function parseCreateResponseInput(
  request: Request,
  draftId: string,
): Promise<{ draftId: string; source: 'manual-paste'; responseMarkdown: string; externalAgentName?: string; responseNote?: string }> {
  if (!request.headers.get('content-type')?.includes('application/json')) throw new AgentResponseValidationError('Content-Type must be application/json')
  const raw = (await request.json()) as unknown
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new AgentResponseValidationError('Request body must be a JSON object')
  const body = raw as Record<string, unknown>
  const source = body.source as string | undefined
  if (typeof source !== 'string' || source !== 'manual-paste') throw new AgentResponseValidationError('source must be "manual-paste"')
  const externalAgentName = typeof body.externalAgentName === 'string' && body.externalAgentName.trim() ? body.externalAgentName.trim() : undefined
  if (body.externalAgentName !== undefined && typeof body.externalAgentName !== 'string') throw new AgentResponseValidationError('externalAgentName must be a string')
  const responseNote = typeof body.responseNote === 'string' && body.responseNote.trim() ? body.responseNote.trim() : undefined
  if (body.responseNote !== undefined && typeof body.responseNote !== 'string') throw new AgentResponseValidationError('responseNote must be a string')
  if (typeof body.responseMarkdown !== 'string') throw new AgentResponseValidationError('responseMarkdown is required and must be a string')
  const responseMarkdown = body.responseMarkdown as string
  if (responseMarkdown.trim().length === 0) throw new AgentResponseValidationError('responseMarkdown must not be empty')
  return { draftId, source: 'manual-paste', responseMarkdown, ...(externalAgentName ? { externalAgentName } : {}), ...(responseNote ? { responseNote } : {}) }
}
