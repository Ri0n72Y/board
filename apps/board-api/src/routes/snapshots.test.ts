...
describe('createSnapshotsRoute', () => {
  ...
  it('exports snapshot markdown from static projection', async () => {
    ...
    expect(exportPayload.data.content).toContain('Depends on CARD-4')
    expect(exportPayload.data.content).toContain('constraint: dependsOn')
    expect(exportPayload.data.content).toContain(`target id: ${target?.id}`)
    expect(exportPayload.data.content).toContain('raw id: asset:deck-system')
